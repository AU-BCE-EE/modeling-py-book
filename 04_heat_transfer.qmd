...
where $\dot{m}_m=$ the mass-based rate of melting ($\gps$), $\dot{m}_m=$ the mass-based rate of vaporization (evaporation) ($\gps$), $\dot{Q}=$ the rate of energy flow into the melting material at its freezing point (W), $h_f=$ heat of fusion ($\Jpg$), and $h_v=$ heat of vaporization ($\Jpg$).
...
Now we will explicitly consider heat loss through two different routes: the exposed coffee surface (through two fluid layers: coffe and air) and the mug sides (two fluids and one solid wall). 
...
{{< include modules/cooling_mod_2m.py >}}
```
