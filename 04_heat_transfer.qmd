...
$$
\dot{Q}_{in} - \dot{Q}_{out} = \frac{dQ}{dt} = \dot{V} \cdot{\rho} \cdot c_p \cdot (T_{inflow} - T_{tank}).
$${#eq-tank1}
...
So we can get a temperature derivative from @eq-tank1 by dividing by $\dot{V} \cdot{\rho} \cdot c_p$: 
...
This equation and those above calculate a temperature difference from left to right because it is conceptually simpler and does not require a negative sign on the RHS.
But it is actually more common to define differences from right to left, or, technically, from farther from the domain origin to closer.
Here, that would be:

$$ 
\frac{dT}{dt} = - v_x \cdot \frac{( T_{x=\Delta x} - T_{x=0} )}{\Delta x}.
$${#eq-advection6}

That quotient on the RHS of @eq-advection6 becomes the definition of a derivative as $\Delta x$ approaches 0, so the associated ODE is:
...
In reality, we could have both differences in longitudinal transfer with radial position (slower flow near the wall) and significant radial resistance to transfer.

In heat transfer models (as well as mass transfer), it is common to work with flow rates normalized by the cross-sectional area of the space through which the flow is moving.
This normalized flow is called heat *flux* in engineering (flux *density* in physics and probably elsewhere).
Our heat flow units are typically W and heat flux $\Wpms$.
...
Another reason is that the consitutive equations are most simply expressed for flux--it is flux not flow that depends on a temperature gradient.
...
@eq-fouriers-law1 says that the heat flux in the $x$ direction is proportional to derivative of temperature with respect to $x$, or the temperature gradient.
The two--heat flux and temperature gradient--are linked through a proportionality constant called *thermal conductivity*, which typically has units of $\WpmpK$ and depends strongly on the substance through which heat is diffusing.
Thermal conductivity is high for metals (around $10^2$  $\WpmpK$), low for gases (around $10^{-2}$ to $10^{-1}$ $\WpmpK$), and somewhere in the middle for other materials.
...
This dependency is a characteristic of constitutive equations.
...
![A temperature profile through a plane wall.](drawings/wall_temp_prof1.png){#fig-wall1 fig-alt="Wall temperature profile."}
...
Applying @eq-fouriers-law1, the heat flux is 50 $\Wpms$ at all locations.
...
Well, the nonlinear temperature profile means the temperature derivative $\frac{dT}{dx}$ is not constant over space, which means that the rate of heat transfer, the heat flux really, is not constant over space either.
At the left side, there is a *positive* temperature derivative, so the heat flux is *negative*.
...
![A different temperature profile through a plane wall.](drawings/wall_temp_prof2.png){#fig-wall2 fig-alt="Wall temperature profile."}
...
For a fixed wall thickness, resistance is inversely proportional to thermal conductivity.
...
In @sec-series, we will see how the resistance approach can be used to combine resistance from multiple layers in a *composite wall*, even including convection.

How you calculate $\Delta T$ and whether your equation needs a negative sign depends on the convention you use in your model: Does a positive energy flow term in your model mean that heat is flowing into or out of the domain?
Pick some fixed temperatures, or imagine a difference, and check, mentally!

## Convection and Newton's law of cooling {#sec-newtons-law}

If you are bald and go outside in Norway in the winter without covering your head (@fig-head1) the mode of heat loss that you experience is called *convection*.
The term is, unfortunately, used in some different ways, but we'll follow @Bergman2013 and use it for a combination of conduction and advection that causes heat transfer between a fluid and a surface.
Temperature gradients driving convection are commonly shown as in @fig-convection-prof1.
It is common to use the symbol $T_\infty$ for the *bulk* fluid temperature, "far away" from the surface.
...
$${#eq-newtons-resistance1}

Here $R$ is just the inverse of $h$:

$$
R = \frac{1}{h}.
$${#eq-newtons-resistance2}


## Resistance in series {#sec-series}
...
![A composite wall, including a layer of concrete and insulation.](drawings/composite_wall1.png){#fig-composite-wall1 fig-alt="Composite wall."}
...
Looking at @eq-newtons-resistance1, you might be able to see that convection and conduction together in series, as in @fig-composite-wall2, can also be combined using the same approach.
...
Remember that $\Delta T$ is the overall temperature difference.
...
Really the two figures could be different interpretations of the same physical system.
The resolution comes from defining $U$ correctly (it is conceptually and numerically different for the two) and using the appropriate temperatures.
We could, in fact, use $\Delta T = T_{l} - T_{r}$ for the system in @fig-composite-wall2, but then 1) we should not include the fluids in the calculation of $U$ and 2) we must use the temperatures at the fluid/solid interfaces ($T_l$ and $T_r$) for calculating flux.
...
## Linking equations {#sec-heat-link}
...
For example, a cube of ice can absorb a large quantity of heat energy without any noticeable change in temperature if it is undergoing melting, a phase change.
The heat energy has not disappeared though--the laws of thermodynamics don't allow that!
Instead we have a conversion between *sensible* and *latent* heat energy.
...
There are only two important parameters, which are physical constants: the heat of fusion $h_f$ and heat of vaporization $h_v$, both  typically $\kJpkg$ or $\Jpg$.
These constants directly link the mass-based rate of phase change to energy flow:

$$
\dot{m}_m = \frac{\dot{Q}} {h_f},
$${#eq-hf-link}

$$
\dot{m}_v = \frac{\dot{Q}} {h_v}, 
$${#eq-hv-link}

where $\dot{m}_m=$ the mass-based rate of melting ($\gps$), $\dot{m}_v=$ the mass-based rate of vaporization (evaporation) ($\gps$), $\dot{Q}=$ the rate of energy flow into the melting material at its freezing point (W), $h_f=$ heat of fusion ($\Jpg$), and $h_v=$ heat of vaporization ($\Jpg$).

## Example: lumped-parameter model with multiple modes
...
Now we will explicitly consider heat loss through two different routes: the exposed coffee surface (through two fluid layers: coffee and air) and the mug sides (two fluids and one solid wall). 
We have a choice of using more fundamental parameter inputs or more empirical.
The more fundamental approach would be to include parameters for liquid phase convection coefficients, conductivity and thickness of the cup itself, and convection coefficients for the air, and calculate both $U$ parameters internally.
We'll go with the more empirical route, where only $U_t$ (t for top) and $U_s$ (s for side) are specified as inputs.
This is done in the module `cooling_mods_2m.py` (`2m` is for "2 modes").
Of course, they could be calculated from the same inputs externally.

![Extended coffee model.](drawings/coffee2modes.png){#fig-coffee2modes fig-alt="Extended coffee model sketch."}

```{.python filename="cooling_mods_2m.py"}
{{< include cooling_mod_2m.py >}}
```
...
1. You have a fermenter that is heated to around 45 $\degC$ using a water jacket. 
   ...
   Develop and implement a steady-state model to predict fermenter temperature based on heating water temperature and relevant parameters and system characteristics.
