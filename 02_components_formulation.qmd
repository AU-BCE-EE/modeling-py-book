...
Air temperature sits outside the boundary, implying it is a fixed external condition.
It could also be called an input variable, following \@sec-components.
(It can be hard to draw the boundary exactly where it should be, but close usually works.)
...
Here, geometry means the shape of the modeled domain.
It could be plane wall or a cylinder for example, and it determines the form that the GE takes (@sec-spatial).
The actual numeric values that quantify the geometry, e.g., length, are themselves parameters.
In our coffee model the geometry is a well-mixed lumped volume, but that can be hard to show clearly in a sketch.
...
Area $A$ is another model input, specifically a parameter.
...
*Conservation equations* or *balance equations* are a fundamental part of all models that we will consider, whether or not the equations are explicitly considered during model formulation.
...
We will typically use rates for all terms, e.g., $\Jps$ = W for energy and $\gps$ or $\kgps$ for mass.
...
![Conceptual model of the Aarhus University WET model (https://projects.au.dk/wet/about-qwet).](images-web/AU_WET1.png){#fig-wet1 fig-alt="Diagram of a lake model."}           
...
This highlights an important point: that conservation equations apply to a specific state variable and a specific *domain*.
...
Fortunately, many of the models we'll cover in this book have a single state variable and so a single conservation equation.
...
We will need either a constitutive equation or a known input value for each term on the RHS of the model conservation equation.
Figuring out what equations to use here requires familiarity with the different options.
Some of the equations may be so simple we might not identify them as constitutive equations--reaction rates and advective flow are two examples.
We'll take a look at just a couple here and present more in @sec-heat, @sec-mass, and @sec-reactions.
Constitutive equations often have a name like "someone's law".
In general, they include a difference or derivative and a proportionality constant on the right-hand side (RHS).
The left-hand side (LHS) is often some kind of *flux*, which is a flow rate per unit cross-sectional area.
In other cases, the LHS may simply be a rate of change in concentration or even mass and the RHS may be a simple product (e.g., advection or reaction rates).
The important thing is to have some way of knowing the value for each term on the RHS of the conservation equation.
...
*Linking equations* are those relationships that are sometimes needed to connect or relate different physical quantities included in the model.
...
where $\Delta Q=$ a change in thermal energy (J), $c_p=$ a new parameter called *specific heat capacity* ($\JpgpK$), $m=$ the material mass (g), and $\Delta T=$ the corresponding change in material temperature (K or $\degC$).
...
2. A tank contains water and a small amount of salt. Saltwater is flowing
into the tank at a constant rate with a known concentration. At the same time the saltwater from the tank flows out from the tank at another outlet. Formulate a model by following the steps in model formulation. Think about what actual information you need to solve the model? 

3. Refrigeration of animal slurry is a commercial technology used at full-scale to reduce emission of methane and other air pollutants from animal barns.
   Using the cooling coffee model for inspiration, follow the model formulation steps to try to develop a governing equation for a slurry cooling system. What if slurry is added to the system over time (animals continuously excrete manure that dumps into the slurry pit).

4. A denitrifying wood chip bioreactor is just a trench in the ground filled with wood chips.
