...
As part of verification, you should carefully check model code for:
...
Even better, you can explicitly add cumulative transport as a state variable, and compare it to the change in total energy or mass in the model domain for a conservation check.
...
Let's start with a conceptual check of
...
For the units check, let's focus on that same line of code from above.
```{python}
# 2. Air/top
#    u_top : float
#        Overall heat transfer coefficient from the upper water surface (W/m2-K)
...
But we have to check that the inputs `a_top` etc. have the correct units. 
These should be defined in comments.
We see:
