...
![Where verification and validation fit into the modeling process. From @thackner_2004](images-web/Thackner_VV1.png){#fig-vv1 fig-alt="V&V concept."}
...
![Graphical validation of a pesticide volatilization model from @yates_simulating_2006. Measurements are shown as points, and model output as lines.](images-papers/pestmod1.png){#fig-pestmod1 fig-alt="Volatilization of a pesticide."}
...
![Graphical validation of a soil microbiology model from @zhang-denit-mod-2023. Each plot shows a different variable. Measurements are shown as points, and model output as lines.](images-papers/denitmod1.png){#fig-denitmod1 fig-alt="Denitrification model validation."}
...
![Residuals or model error shown as dotted gray lines between model output (fuchsia line) and measurements (blue points).](drawings/fit_stats0.png){#fig-fit-stats0 fig-alt="Model residuals or error"}
...
![Demonstration of two different model fits. Measurements shown as point, model output as lines. Output B has a negative MBE (bias) because it underpredicts almost every measurement point. MBE for A could be smaller (closer to zero). But the overall fit is better for B, and this would be reflected in lower values for MAE and RMSE. Output C has a high (positive) MBE and large (poor) MAE and RMSE.](drawings/fit_stats1.png){#fig-fit-stats1 fig-alt="Fit statistics demo"}
...
![The same demonstration of model fit as in @fig-fit-stats1 but only showing the model output used for calculating fit statistics, i.e., perfect alignment of measurements and model output.](drawings/fit_stats2.png){#fig-fit-stats2 fig-alt="Fit statistics demo with points"}

## Problems {.unnumbered}

### Verification {.unnumbered}

#### Ammonia emission model {.unnumbered}
...
1. Can you verify the model following the approach we went through in class? 
...
1. See the data file `col_salt.csv` for some (fabricated) measurements of the total salt (NaCl here) mass within a 0.1 m long column with a diameter of 0.02 m. 
