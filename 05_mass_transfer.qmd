...
For example, values of $D$ for oxygen in air and water are of course different, but so are the values for carbon dioxide in air and ethanol in air. 
...
The time derivative of concentration $\frac{dc}{dt}$ is proportional to the second space derivative:
$$ 
\frac{dc}{dt} = D \cdot \frac{d^2c}{dx^2}.
$${#eq-ficks2}
This form can be used as a starting point for a numerical or analytical solution.
Note the difference from advection--this can be conceptually confusing!
...
Instead, all resistance to mass transfer is lumped into a single convection coefficient $k_c$.
...
The discontinuity at the interface between the two phases is a defining feature of interphase mass transfer (note that interface and interphase are two different words with two different meanings!).
...
The straight lines with $\frac{dc}{dz} = 0$ show that there is no mass transfer--you should be able to infer this result from @eq-ficks1.
...
Oxygen is relatively insoluble in water, so even though air is around 20% oxygen by volume, the corresponding equilibrium concentration in water is around 10 $\mgpl$ (= 10 $\gpmc$).
...
An overall interphase mass transfer coefficient can also be expressed in gas phase units.
...
We won't get into explicitly modelling concentration profiles until the spatial sections later in this book.
Here we may think about the shape of the profiles, but we won't try to quantitatively describe them--that has to wait for @sec-spatial. 
