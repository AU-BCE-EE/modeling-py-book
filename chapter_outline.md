...
# 2. Model components and formulation (Sasha)
Introduce the building blocks of a model and show how they are assembled to produce a governing equation.
Bring a single example (cooling coffee) through all parts.

## Conservation equations
Solid introduction to mass and energy balances and show how a simple "model" (or at least some system insights) can be developed from just a single conservation equation.
...
* Conservation equations apply to some volume or domain, typically (1) entire domain for lumped parameter or 0D models or (2) a cell or node volume for 1D+ models or (3) an arbitrary "control volume" for symbolic math, i.e., developing a governing equation
* Include some examples:
    - Balance equation as start of model dev
    - Balance equation that can be used alone for insight or a simple "model"

## Constitutive equations
Introduce concept of constitutive equations and how they are a component of model development.
* Why rates matter
* Example with Fourier's law (brief preview; full treatment in Chapter 4)
* Constitutive equations often have a derivative or difference on the RHS
* Constitutive equations depend on material properties and state variables

## Linking equations
* Connect different physical quantities (e.g., energy and temperature)
* Needed when state variable in balance equation differs from state variable of interest

## Model formulation
Show how models are formulated by combining balance, constitutive, and linking equations to arrive at the governing equation (GE).
Scope here is 0D (lumped-parameter) models.
The approach for 1D+ models (starting from a complete GE and simplifying) is introduced in Chapter 7.
...
* Solution deferred to Chapter 3


# 3. Solving and implementing models (Sasha)
Present methods for solving a governing equation, both numerically and analytically.
Continue with the coffee example throughout.

## Going from GE to code
* Lots of variation in exactly how to do this
* Numerical implementations can use separate balance and constitutive equations without even going to a single GE
* Single GE can always be basis for numerical or analytical solutions and implementations
* Spatial models will take some more work--see Chapter 7
...
* Examples from Part II (forward references)
...
> *[Note: no content yet; this was a full class in the course (class 5) -- likely needs more depth than other subsections]*
...
# 6. Chemical reactions and microbiology (Sasha)
Brief introduction to reaction kinetics and microbial growth as sources/sinks in mass and energy balances.
...
* Rate of something over something else over infinitesimally small intervals (d = differential = symbol (not variable) that means "a very small change in ...")
* Examples:
    dT/dt = rate of change of temperature over time
    dT/dx = *gradient* in temperature over space
* ODEs
* PDEs are *partial* derivatives--the "partial" (or del) symbol $\partial$ implies that the variable on the top also depends on other variables
