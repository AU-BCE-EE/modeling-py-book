...
**Implementation in Python**{#ex-pipe_model}
...
2\) When we are dealing with a symmetric problem where $x = 0$ is a boundary e.g. in the middle of a sphere or cylinder that is symmetric around $x = 0$. This is actually what is done for BC2 in @fig-bcs where $r_0$ is in the middle of the cylindrical pipe in @fig-pipedisc.  
...
$$
\frac{\partial T}{\partial t} = \frac{k}{\rho \cdot C_p}\cdot \frac{T(x_{L+1}) - 2T(x_L) + T(x_{L-1})}{\Delta x^2}
$$
...
This equation enforces the Neumann boundary condition at the top boundary, L. Earlier in @ex-pipe_model, we avoided the whole ghost point problematic by using backward difference instead. But that backward difference code silently also imposed a Neumann BC of zero flux. In that example it was the \3 case, where nothing was specificed about the dynamics of the state variable at the other side of the boundary.
