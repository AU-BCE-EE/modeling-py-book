...
backward difference to avoid that.
...
\~11 seconds. Se see that the heat source only increases the temperature
...
boundary to $10 \degC$. The Dirichlet BC can be mathematically
formulated as:
...
it is enforced by setting the initial condition equal to the BC. Inside
...
index, it means that we also have a Dirichlet BC there, which is 0 from
this command: `c0 = np.zeros(len(x_grid))`. This may not be what we want
for the model, unless something is instantly consuming oxygen at the
bottom of the pond. Therefore we might have to handle the bottom
...
gradient. The Neumann BC can in general by formulated as:
...
around $x = 0$. This is actually what is done for BC2 in @fig-bcs where
$r_0$ is in the middle of the cylindrical pipe in @fig-pipedisc.

3\) When we have a completely open boundary, but nothing is specified
about the dynamics of the state variable at the other side of the
boundary. Typically there would be no description of convection or
...
problematic by using backward difference instead. But that backward
difference code silently also imposed a Neumann BC of zero flux. In that
example it was the case 3) where nothing was specificed about the
dynamics of the state variable at the other side of the boundary.
