...
$$
-k\frac{\partial T(x = a, t)}{\partial t}=h \cdot(T(x=a, t) - T_{bulk})
$$
...
$$
-D\cdot \frac{\partial c(x = L, t)}{\partial t}= -D\cdot \frac{c(x_{L+1}) - c(x_{L-1})}{2 \cdot \Delta x} = k_c \cdot(c(x_L) - c_{bulk})
$$
...
$$
D \cdot \frac{c(x_{L+1}) - 2\cdot c(x_L) + c(c_{L-1})}{\Delta x^2} = D \cdot \frac{\left(-\frac{2 \cdot \Delta x\cdot k_c}{D} \cdot \left( c(x_L) - c_{bulk} \right) + c(x_{L-1})\right ) - 2\cdot c(x_L) + c(c_{L-1})}{\Delta x^2}
$$
