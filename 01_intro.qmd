...
State variables, parameters, inputs, code. . . what do these terms mean?
Vocabulary is a key part of a new topic, and modeling vocabularly is particularly tricky because terminology is not used consistently among different modeling fields.
...
In an epidemiological model of disease spread, the number of infected and healthy individuals might be the state variables.
...
Model inputs inclue the times for which the solution is desired, one or more physical constants that affect heat transfer, and an initial value of the state variable, coffee temperature.
...
The term can be confusing because the same word, parameters, is used for inputs to a Python function (*arguments* is used in other programming languages).
...
Here is an implementation of the coffee model, i.e., a computer model for cooling coffee. Actually that code applies the model as well, carrying out the model calculation (the `Tc = ...` line) using specific values for input variables and parameters.
