...
A model is linear if it satisfies the superposition principle: scaling the inputs scales the output by the same factor, and the response to a sum of inputs equals the sum of the individual responses. 
Formally, for an ODE system it means that the equations can be written as:

$$
y' = A \cdot y + b
$$

where A is a constant coefficient matrix and b is a constant vector (independent of y). 
Critically, this requires that no state variable appears multiplied by another state variable, raised to a power other than 1, or passed through a nonlinear function (e.g. $\sin(y)$). 
However, independent variables does not have to obey these rules for the system to still be linear. 
Why does this distinction matter so much? 
Several subjects withing mathematical modelling only hold — or only hold exactly — for linear systems:
...
* Superposition-based intuition: in linear systems you can analyze the effect of multiple inputs independently and sum the results. This is not the case for nonlinear systems.
