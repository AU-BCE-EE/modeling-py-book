...
It is important to recognize that there are different types of model inputs.
...
They might also be completely empirical values that quantify some rate of change or anything else that is important in a model. 
Heat or mass transfer coefficients are common model parameters.

*Differential equations* (DEs) are symbolic equations that describe how a dependent or state variable changes with respect to another, independent variable.
A dynamic model, which describes how a dependent variable changes over time are built around a DE that has, of course, *time*, as the independent variable.
If an equation shows that the state variable depends only on time, or some other independent variable, but only one, it is called an *ordinary differential equation* (ODE).
If the state variable depends on more than one independent variable, we have to work with what are called *partial differential equations* (PDEs).
In reality, few things depend on only one variable, but it is a common type of model simplification to focus on one.
The distinction between independent variables, parameters, and other model inputs is important here.
The lake phosphorus model could include effects of temperature and animal activity, but if the fundamental DE used to develop the model only expresses the state variable phosphorus as a derivative with respect to time ($\frac{dP}{dt} =...$) then it is an ODE, not a PDE.
