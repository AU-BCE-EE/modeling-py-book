...
A model is linear if it satisfies the superposition principle: scaling the inputs scales the output by the same factor, and the response to a sum of inputs equals the sum of the individual responses. Formally, for a an ODE system it means that the equations can be written as:

$$
y' = Ay+b
$$
where A is a constant coefficient matrix and b is a constant vector (independent of y). Critically, this requires that no state variable appears multiplied by another state variable, raised to a power other than 1, or passed through a nonlinear function (e.g. $\sin(y)$). However, independent variables does not have to obey these rules for the system to still be linear. 

Why does this distinction matter so much? Several subjects withing mathematical modelling only hold — or only hold exactly — for linear systems:

\cdot Analytical solvability: linear ODE systems can often be solved using matrix methods as in the formulation above, whereas nonlinear systems generally needs to be solved with numerical methods. 

\cdot Stability analysis: for linear systems, stability is determined globally, whereas for nonlinear systems stability must be assessed locally and in many places. 

\cdot Superposition-based intuition: in linear systems you can analyze the effect of multiple inputs independently and sum the results. This is not the case for nonlinear systems.
...
1. A hypothetical geospatial water quality model used to predict nutrient concentrations dissolved oxygen in the river and estuary near a major urban location in Europe incorporates many assumptions, including uniform temperature and solute concentrations over the vertical dimension. 
   ...
   Who is right?
   Discuss.
   Be sure to consider the concept of model simplification.

2. A fellow student claims that analytical modelling is outdated and completely illogical in the present age of cheap computing power.
   Do you agree?
   Explain.
