...
There are two general ways to do that: analytical and numerical approaches.
The conceptual difference between the two of these should be familiar from @sec-analytical-v-numerical.
In this chapter we'll explore the two types of models, an methods of solving models, in more detail.
...
In contrast, analytical models could be applied with some simple calculations and so creating a Python script or function isn't always necessary for model application.
A calculator or even pencil and paper could be sufficient.
But in this book, we will also implement analytical models in Python, to make application easier.
...
These types of problems are often known as initial value problems (IVP) for this reason.
...
It was first described by Leonhard Euler more than 250 years ago.
...
It is good practice to try some different values to see the effect.
...
We will generally use this approach for all the following computer models in this book.
...
Now we can call the function and run the model with a single functin call (could be a single line of code, but we'll split it across multiple lines for clarity).
...
This type of *oscillating* response is an example of *numerical instability*, and reflects inaccuracy in the solution as well as solutions that depend on the details of the call.
Here, the solution depends strongly on the time step.
...
The `ivp` bit is for IVP, initial value problem.
...
The first of these means that we, the users, don't set a fixed step size `dt`.
...
The slopes considered in the 4th order Runge-Kutta method are in @fig-RK4-slopes.
And the green asterisk shows the estimated value, which is quite close to the true response shown by the blue curve.
Where would the Euler method land?
(Hint: at $t_0 + h$, and look at $k_1$ for the single slope that is applied.)

![The slopes used in the classical Runge-Kutta method. $h=$ the time step. By HilberTraum <https://commons.wikimedia.org/w/index.php?curid=64366870>.](images-web/Runge-Kutta_slopes.png){#fig-RK4-slopes fig-alt="Runge-Kutta slopes."}

So what does `solve_ivp()` look like in practice?
In a way it is simpler to set up than the Euler method, because `solve_ivp()` takes care of the iterative bit that we have to program explicitly using a for loop.
...
There are other good approaches, but this one makes self-contained model functions and work well.
...
Let's see how it works, and compare it to the first solution from the Euler method.
...
As soon as a model becomes a bit complicated analytical solutions become impossible.
Even in these cases though, it can be possible to derive an analytical solution to a limiting case, often steady-state.
There are many different ODEs that can be solved analytically.
Here we will work with only one, which appears in many different applications, including our cooling coffee model: the exponential model.
...
1. In first-order chemical reactions, where $x$ might be the concentration or mass of a reactant and $\lambda$ is entered as the negative of the first-order rate constant. 
...
The $x_0$ constant turns out to be the value of $x$ at time $0$, which is why we gave it that symbol.

Now, let's check the solution using a general approach.
We'll differentiate it and compare it with the original ODE @eq-exp-ode.

So, we want:

$$
\frac{d ( x_0 \cdot e^{\lambda \cdot t} )}{dt}.
$$

You can check a table of derivatives to see that the derivative of any $a \cdot e^{b \cdot t}$ with respect to $t$ is just $b \cdot a \cdot e^{b \cdot t}$:

$$
\frac{d (x_0 \cdot e^{\lambda \cdot t} )}{dt} = \lambda [ \cdot x_0 \cdot e^{\lambda \cdot t} ]. 
$$

And the part in square brackets $[ ~~ ]$ above is the solution to $x$, we see that in fact,

$$
\frac{d (x_0 \cdot e^{\lambda \cdot t} )}{dt} = \lambda \cdot x. 
$$
...
$$
\theta = \theta_0 \cdot e^{-k_{cool} \cdot t}.
$${#eq-cooling-an-sol}

So this is our solution--an equation that gives us the coffee temperature at any time depending on the inputs that go into the $k_{cool}$ term.
...
1. Script
    ...
    * Model may be written using functions (as in most examples above) or without (as in first example)
    * Simple and quick
...
Where M is the mass of gasoline and k is the evaporation rate constant. You should try to make both a numerical and analytical model and check that predictions match
...
You should try to make a numerical model for this problem. If you are up for it you can also try to derive the analytical solution and implement it or you can look it up in a algebra book.
...
4. Assume you have a big water cubic tank, say 10 $\text{m}^3$, that is draining through a pipe at the bottom under the force of gravity.
   You measure an initial flow rate of 100 $\text{L}~\text{min}^{-1}$ when the tank is full.
   If you assume that flow rate is proportional to the tank pressure, which is not an unreasonable assumption (but not always true), then it is also proportional to the quantity of water remaining in the tank (remember this is a cubic tank, i.e., water volume and water height are directly proportional).
   Develop a model, analytical or numerical, to calculate the volume of water remaining in the tank at any time.
